...
working_dir = "/home/pranshu/Downloads/work/3c84_2021/"
...
# bc.save_fsp_pickle(mkid_files, working_dir + "3c84_2021", mode="single_col") #mode can be "average" of "single_col"

data = bc.load_pickle(working_dir + "3c84_2021.pickle")
data = np.array(data)
print(len(data))
print(data.shape)
...
clipped = bc.clip(data, sample_rate * 200, -sample_rate * 70) # clip for decorrelation
...
# bc.save_as_pickle(final_flat, working_dir + "final_flat_selected")

cleaned = bc.load_pickle(working_dir + "final_flat_selected.pickle")
...
for i in range(len(cleaned)):
    running_avged.append(np.convolve(cleaned[i], np.ones((N,))/N, mode='same'))
print(np.shape(running_avged))
running_avged = normalize(running_avged)

pix = 41
plt.plot(running_avged[pix], color='mediumseagreen')
plt.xlabel('Time(sec)', fontsize='18')
plt.ylabel('Centered Frequency shift(MHz)', fontsize='18')
# plt.savefig('first_pca_baseline.eps')
plt.show()
